...
func handleContracts(st *store.Store, _ []query.Option, renderOpts []render.Option, args map[string]any) (string, bool) {
	...
	if err != nil {
		return fmt.Sprintf("Error querying contracts: %s", err), true
	}

	data, err := json.MarshalIndent(contracts, "", "  ")
	if err != nil {
		return fmt.Sprintf("Error marshaling contracts: %s", err), true
	}
...
func handleContractDrift(st *store.Store, _ []query.Option, renderOpts []render.Option, args map[string]any) (string, bool) {
	...
	if err != nil {
		return fmt.Sprintf("Error querying drift: %s", err), true
	}

	data, err := json.MarshalIndent(drifts, "", "  ")
	if err != nil {
		return fmt.Sprintf("Error marshaling drift: %s", err), true
	}
	return string(data), false
}
...
func handleRuntimeContracts(st *store.Store, _ []query.Option, renderOpts []render.Option, args map[string]any) (string, bool) {
	...
	if err != nil {
		return fmt.Sprintf("Error querying runtime contracts: %s", err), true
	}

	data, err := json.MarshalIndent(contracts, "", "  ")
	if err != nil {
		return fmt.Sprintf("Error marshaling runtime contracts: %s", err), true
	}
	return string(data), false
}

func handleSuppressContract(st *store.Store, _ []query.Option, renderOpts []render.Option, args map[string]any) (string, bool) {
	fromRef, ok1 := args["from_ref"].(string)
	toRef, ok2 := args["to_ref"].(string)
	if !ok1 || !ok2 {
		return "Error: from_ref and to_ref are required", true
	}

	if err := st.SuppressContracts(fromRef, toRef); err != nil {
		return fmt.Sprintf("Error suppressing contract: %s", err), true
	}

	return fmt.Sprintf("Suppressed contract %s → %s", fromRef, toRef), false
}
